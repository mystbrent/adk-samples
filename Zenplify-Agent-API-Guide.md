...
#### Direct LLM Suggestion (New)
- **URL**: `/api/unidentified-fields/direct-suggest/`
- **Method**: `POST`
- **Description**: Generates a suggestion directly using the LLM based on the user's profile and provided context, without searching Q&A history first. Useful for questions requiring unique, context-aware answers.
- **Query Parameters**:
  - `generate_alternatives` (boolean, optional, default: `false`): If true, generates alternative suggestions using the LLM.
- **Request Body**: 
  ```json
  {
    "user_id": "7f8d3e2c-9a6b-4c5d-8e7f-1a2b3c4d5e6f",
    "question": "Describe your experience with scaling distributed systems.",
    "context": {
  ...
  ```
  > **Note**: `user_id` and `question` are required fields.
  
- **Response (200 OK)**: (Similar format to `Suggest for Unidentified Field`)
  ```json
  {
    "suggestion": "In my previous role at TechGlobal, I was responsible for scaling our primary data ingestion pipeline to handle a 10x increase in traffic. This involved optimizing our Kafka cluster configuration, implementing horizontal pod autoscaling in Kubernetes, and refactoring critical microservices for better concurrency. As a result, we achieved a 99.99% uptime and reduced average message processing latency by 40%.",
    "confidence": 0.88, 
    "source": "llm_generation",
    "alternative_suggestions": [] // Would be populated if generate_alternatives=true
  }
  ```
- **Error Responses**:
  - 400 Bad Request: Missing required fields or invalid format
  - 404 Not Found: User not found
  - 500 Internal Server Error: Failed to generate suggestion via LLM

#### Batch Suggest for Unidentified Fields (New)
- **URL**: `/api/unidentified-fields/suggest/batch/`
- **Method**: `POST`
- **Description**: Processes multiple requests for unidentified field suggestions concurrently for a single user. It attempts to find answers in Q&A history first, then falls back to LLM generation based on user profile if necessary. Faster than making individual requests.
- **Request Body**: 
  ```json
  {
    "user_id": "7f8d3e2c-9a6b-4c5d-8e7f-1a2b3c4d5e6f",
    "requests": [
      {
        "field_label": "Why do you want to work for our company?",
        "context": { "company": "Acme Inc", "job_title": "Software Engineer" }
      },
      {
        "field_label": "What are your salary expectations?",
        "context": { "company": "Acme Inc", "job_title": "Software Engineer" }
      },
      {
        "field_label": "Describe a time you failed.",
        "context": { "company": "Acme Inc" }
      }
    ],
    "generate_alternatives": false // Optional: applies to all LLM fallbacks in the batch
  }
  ```
  > **Note**: `user_id` and the `requests` array are required. Each item in `requests` requires `field_label`.
  
- **Response (200 OK)**: Returns a list where each item corresponds to an input request. Items can be a successful suggestion (`UnidentifiedFieldSuggestion`) or an error (`SuggestionError`).
  ```json
  {
    "results": [
      {
        "suggestion": "I'm interested in working for Acme Inc because...", // Found in QA history
        "confidence": 0.95,
        "source": "qa_history",
        "alternative_suggestions": []
      },
      {
        "suggestion": "Based on my experience and the industry standard for this role in this location, I am seeking a salary in the range of $X to $Y.", // Generated by LLM
        "confidence": 0.75,
        "source": "llm_generation",
        "alternative_suggestions": []
      },
      {
        "field_label": "Describe a time you failed.", // Identifier for the failed request
        "error": "Suggestion generation failed",
        "details": "LLM service returned an error: Rate limit exceeded"
      }
    ]
  }
  ```
- **Error Responses (Overall Batch)**:
  - 400 Bad Request: Invalid request structure (e.g., missing user_id)
  - 404 Not Found: User not found (prevents processing the entire batch)
  - 500 Internal Server Error: Unexpected error during batch processing (individual errors are handled within the `results` list)

#### Batch Direct LLM Suggestion (New)
- **URL**: `/api/unidentified-fields/direct-suggest/batch/`
- **Method**: `POST`
- **Description**: Generates suggestions directly using the LLM for multiple questions concurrently for a single user, based on their profile and provided context. Does not check Q&A history.
- **Request Body**: 
  ```json
  {
    "user_id": "7f8d3e2c-9a6b-4c5d-8e7f-1a2b3c4d5e6f",
    "requests": [
      {
        "question": "How would you approach designing a system for real-time analytics?",
        "context": { "job_title": "Data Architect" }
      },
      {
        "question": "What is your experience with cloud-native monitoring tools?",
        "context": { "company": "CloudCorp" }
      }
    ],
    "generate_alternatives": true // Optional: applies to all requests in the batch
  }
  ```
  > **Note**: `user_id` and the `requests` array are required. Each item in `requests` requires `question`.
  
- **Response (200 OK)**: Returns a list similar to the batch suggest endpoint, containing `UnidentifiedFieldSuggestion` or `SuggestionError` objects.
  ```json
  {
    "results": [
      {
        "suggestion": "I would start by defining the key metrics and latency requirements... involving technologies like Kafka, Flink, and Druid...",
        "confidence": 0.90,
        "source": "llm_generation",
        "alternative_suggestions": [
          "My approach involves using a stream processing engine like Apache Flink...",
          "For real-time analytics, I'd propose a lambda architecture..."
        ]
      },
      {
        "suggestion": "I have extensive experience with Prometheus, Grafana, and the ELK stack...",
        "confidence": 0.85,
        "source": "llm_generation",
        "alternative_suggestions": [
          "In my previous projects, I utilized Datadog for comprehensive cloud monitoring...",
          "My expertise includes setting up and managing monitoring solutions using cloud provider tools like CloudWatch..."
        ]
      }
    ]
  }
...
- **Error Responses (Overall Batch)**:
  ...
  - 404 Not Found: User not found
  - 500 Internal Server Error: Unexpected error during batch processing
